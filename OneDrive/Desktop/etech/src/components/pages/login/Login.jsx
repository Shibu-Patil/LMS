...
const Login = () => {
  ...
  let {globalState:{auth},setGlobalState}=useContext(Contextc)
  ...
  let handelSubmit=(e)=>{
    e.preventDefault();
      try {
       ...
       (async()=>{
        let {success,token}=await services.checkLogin(payload)
    ...
    <Section>
      ...
      <Aside style={{paddingTop:'50px'}}>
        <main style={{lineHeight:'1px'}}>
          <picture>
            <span style={{height:'40px', width:'40px', fontSize:'50px'}}>👋</span>
          </picture>
          <H1>Login into Eduport!</H1>
          <Para>Nice to see you! Please log in with your account.</Para>
        </main>
        <ContainerMain>
          <Form action="" onSubmit={handelSubmit}>
            <>
              ...
              <div style={{margin: '30px 0px -20px 0px',fontSize:'16px'}}>
                <input type="checkbox" name="" id="" onChange={handelCheck} checked={checked} /><span>Remember me<a href="" style={{color:'#9A9EA4', position:'relative', left:'250px'}}>Forget Password?</a></span>
              </div>
              <FormDiv>
      ...
      <ButtonsDiv>
        ...
        Already have an account?
          <Link to={'/register'} style={{textDecoration:'none',color:'#1472CC',letterSpacing:'0.5px'}}>Sign up here</Link>
        </DivLink>
      </Aside>
      <Toaster></Toaster>
    </Section>
